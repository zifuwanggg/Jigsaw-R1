...
class MLLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        model: Union[str, PreTrainedModel],
        reward_funcs: Union[RewardFunc, list[RewardFunc]],
        reward_weights: Union[float, list[float]],
        ...
        score_jigsaw_connections: bool = False,
        ...
        **kwargs,
        ...
        self.mllm_module.pre_model_init(model_init_kwargs)
        model_cls = self.mllm_module.get_model_class(model_id)
        model = model_cls.from_pretrained(model_id, **model_init_kwargs)

        # LoRA
        self.vision_modules_keywords = self.mllm_module.get_vision_modules_keywords()
        ...
        if processing_class is None:
            processing_cls = self.mllm_module.get_processing_class(model_id)
            processing_class = processing_cls.from_pretrained(model_id, trust_remote_code=model_init_kwargs.get("trust_remote_code", None))
            for component, processing_keyword in self.mllm_module.get_custom_processing_keywords():
        ...
        self.mllm_module.post_model_init(model, processing_class)
        self.mllm_module.post_model_init(self.ref_model, processing_class)
        ...
        self.score_jigsaw_connections = score_jigsaw_connections
        ...
        if hasattr(self.mllm_module, "get_eos_token_id"): # For InternVL
            self.generation_config.eos_token_id = self.mllm_module.get_eos_token_id(processing_class)
        ...
        self.reward_weights = self.reward_weights.to(device)
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        text_inputs = self.mllm_module.prepare_text_inputs(self.processing_class, prompts)
        image_inputs = self.mllm_module.prepare_image_inputs(prompts)
        widths = [image.size[0] for image in image_inputs]
        heights = [image.size[1] for image in image_inputs]

        prompt_inputs = self.mllm_module.prepare_prompt_inputs(
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            generate_returned_result = unwrapped_model.generate(
                **{k: v for k, v in prompt_inputs.items() if k not in self.mllm_module.get_non_generate_params()},
                generation_config=self.generation_config
            )
            prompt_length = prompt_ids.size(1)
            if not self.mllm_module.is_embeds_input():
        ...
        multimodal_keywords = self.mllm_module.get_custom_multimodal_keywords()
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                reward_kwargs = {key: [] for key in inputs[0].keys() if key not in ["completion"]}
                for key in reward_kwargs:
                    for example in inputs:
                        # No need to duplicate prompts as we're not generating multiple completions per prompt
                        # reward_kwargs[key].extend([example[key]] * self.num_generations)
                        reward_kwargs[key].extend([example[key]])
                reward_kwargs["width"] = widths
                reward_kwargs["height"] = heights
                reward_kwargs["score_jigsaw_connections"] = self.score_jigsaw_connections
        ...
        rewards_per_func *= self.reward_weights
        ...
        elif self.reward_reduction == "none":
            advantages = torch.zeros(rewards_per_func.size(0), device=device)
            for reward_idx in range(rewards_per_func.size(1)):
                rewards = rewards_per_func[:, reward_idx]
                ...
                std_grouped_rewards = std_grouped_rewards.repeat_interleave(self.num_generations, dim=0)
                advantages += (rewards - mean_grouped_rewards) / (std_grouped_rewards + 1e-4)
            advantages = advantages / rewards_per_func.size(1)

            # re-compute rewards for metrics
            rewards = rewards_per_func.sum(dim=1).detach().clone()
            mean_grouped_rewards = rewards.view(-1, self.num_generations).mean(dim=1)
            std_grouped_rewards = rewards.view(-1, self.num_generations).std(dim=1)
        else:
            raise ValueError(f"Invalid reward reduction: {self.reward_reduction}")
